...
* GenerateBlocks Pattern Library discovery, pattern search, and native required-Global-Style import using the same source as the editor, including custom/local libraries
...
= 1.1.66 =
* Check permission to edit each attachment before regenerating its image files or metadata.
...
* Recognize Global Style classes inside compound native selectors when guarding page saves.
...
* Resolve trusted style data by library pattern ID and preserve explicit site values on existing classes.
...
= 1.1.56 =
* Support current GenerateBlocks compound Global Style selectors through the same native synchronization interface.

= 1.1.55 =
* Compile current GenerateBlocks Global Style CSS from the native style data instead of requiring callers to duplicate it.
* Synchronize selectors from one native style inventory read.
...
= 1.1.52 =
* Fixed: targeted GenerateBlocks cache invalidation no longer performs an unrelated full-site content scan.
...
= 1.1.39 =
* Supply GenerateBlocks design markers through the vendor-neutral Content Write Gate Adapter.
