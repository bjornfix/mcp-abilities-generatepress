...
[![Release 1.1.62](https://img.shields.io/badge/release-1.1.62-blue.svg)](https://downloads.devenia.com/mcp-abilities-generatepress.zip)
...
| `generatepress/list-overlay-panels` | List native GenerateBlocks Pro Overlay Panels |
| `generatepress/upsert-overlay-panel` | Idempotently create or update a native GenerateBlocks Pro Overlay Panel |
| `generatepress/attach-menu-item-mega-menu` | Attach a published mega-menu Overlay Panel to a WordPress menu item |
...
| `generateblocks/get-global-styles` | Get current GenerateBlocks Pro Global Styles |
| `generateblocks/update-global-styles` | Upsert or explicitly delete current GenerateBlocks Pro Global Styles |
...
The response uses the current GenerateBlocks Pro global-class model. Each item includes its native selector, style data, generated CSS, status, and menu order.

### Update GenerateBlocks global styles

```json
{
  "ability_name": "generateblocks/update-global-styles",
  "parameters": {
    "global_styles": [
      {
...
The adapter compiles the native style data into GenerateBlocks' generated CSS field and rebuilds its external global stylesheet.
...
- Supplies published native GenerateBlocks Global Style CSS through the provider-neutral Block Editor design-context filter.
- Recognizes Global Style classes inside compound native selectors when guarding page saves.

### 1.1.61

- Pattern Library style import now uses the selected library's native authentication headers and verifies the persisted Global Styles.
- Published page writes now require every referenced GenerateBlocks Global Style to be published with generated CSS.
...
- Supports current GenerateBlocks compound Global Style selectors through the same native synchronization interface.

### 1.1.55

- Compiles the required GenerateBlocks Global Style CSS cache from native style data, so callers no longer maintain duplicate CSS.
- Reads the current native style inventory once during synchronization.
...
- Exact Block Element upserts perform no writes or cache invalidation, while changed Elements invalidate dynamic CSS without a synchronous site-wide rebuild.
